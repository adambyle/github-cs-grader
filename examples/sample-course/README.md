# Web Programming (cs108)

This folder is the whole course as the `cs108` command sees it. Keep it in a
...
```bash
cs108 new a04               # make the folders, choose auto or manual
cs108 verify a04            # check it before anyone sees it
cs108 template a04 --go     # starter -> GitHub template repository
...
Nothing changes GitHub without `--go`. Run `cs108` on its own to see what
is out of step, and `cs108 doctor` when something feels wrong.
